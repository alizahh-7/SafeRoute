...
export default function LocationAutocomplete({ value, onChange, placeholder }: Props) {
  ...
  const selectSuggestion = (label: string) => {
    if (debounceRef.current) clearTimeout(debounceRef.current);
    // A suggestion is a replacement value, never text to append to the
    // existing controlled input. Collapse whitespace from provider labels
    // before sending a single clean place name to the parent.
    onChange(label.replace(/\s+/g, " ").trim());
    setSuggestions([]);
    setOpen(false);
    setLoading(false);
  };
      ...
      {open && (loading || suggestions.length > 0) && (
        <div className="absolute z-30 top-full left-0 right-0 mt-space-2xs bg-surface-container-lowest border border-surface-variant rounded-xl shadow-lg overflow-hidden max-h-64 overflow-y-auto">
          ...
          {!loading && suggestions.map((s, i) => (
            <button key={i} type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => selectSuggestion(s.label)}
